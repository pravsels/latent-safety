  ...
  def sample_random_state(
    ...
    while flag:
      ...
      if self.use_wm:
        ...
        rand_u0 = np.zeros(3)
        rand_int0 = np.random.randint(0, 3)
        rand_u0[rand_int0] = 1  
        ...
        feat = self.wm.dynamics.get_feat(prior).detach()
        print(feat.size())
        g_x = self.safety_margin(feat[:, [-1], :])
        print(self.safety_margin(feat[:, [0], :]))
        print(g_x)
        print(state0)
        print(state1)        
        print(state0[0]**2 + state0[1]**2 - 0.5**2)
        print(state1[0]**2 + state1[1]**2 - 0.5**2)
        exit()
      else:
