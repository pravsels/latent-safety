...
class avoid_DDPGPolicy_annealing_dinowm(BasePolicy):
    ...
    def learn(self, batch: Batch, **kwargs: Any) -> Dict[str, float]:
        ...
        if not self.warmup:
            for _ in range(self.actor_gradient_steps):
                ...
                actor_loss = -self.critic(batch.obs, act).mean() + 0.05*rot_cost.mean() + 0.0005*xyz_cost.mean()
                self.actor_optim.zero_grad()
                actor_loss.backward()
                self.actor_optim.step()
        else:
            actor_loss = torch.tensor(0.0)
            rot_cost = torch.tensor(0.0)
            xyz_cost = torch.tensor(0.0)
        ...
        return {
            ...
            "loss/position": xyz_cost.mean().item(),    
    ...
    def exploration_noise(self, act: Union[np.ndarray, Batch],
        ...
        if self.new_expl:
            ...
            rand_act =  act + self._noise(act.shape)
