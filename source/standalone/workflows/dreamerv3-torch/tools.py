...
def evaluate_V(state, policy, mode='ddpg'):
...
def simulate_vectorized(
    ...
    while (steps and step < steps) or (episodes and episode < episodes):
        ...
        obs_vec, reward_vec, done_vec, _ = envs.step(action)
        ...
        for i in range(len(reward)):
            results.append((obs[i], reward[i], done[i], {'discount': np.array(1.0).astype(np.float32), 'safety_score': safety_score}))
        ...
        for a, result in zip(action, results):
            ...
            transition["reward"] = r
            transition["discount"] = info.get("discount", np.array(1 - float(d)))
            transition["safety_score"] = info.get("safety_score") if info.get("safety_score") is not None else 1.
...
def fill_expert_dataset(config, cache, is_val_set=False):
    dataset_path = '/home/kensuke/IsaacLab/logs/robomimic/Isaac-Lift-Cube-Franka-IK-Rel-v0/hdf_dataset.hdf5'
    ...
    pixel_keys = ['wrist_cam', 'front_cam']
    state_keys = ['eef_pos']
    ...
    for i, demo in tqdm(
        ...
        for t in range(len(traj["obs"][pixel_keys[0]])):
            ...
            if len(state_keys) != 0:
                ...
                transition["policy"] = np.concatenate(
                    curr_obs_state_vec, dtype=np.float32
                )

            transition["failure"] = np.array(traj["obs"]["failure"][t],
                dtype=np.float32,
            )
            ...
            transition["reward"] = np.array(traj["rewards"][t], dtype=np.float32)
    ...
    #obj_size = np.size(traj["obs"]["failure"][t])
